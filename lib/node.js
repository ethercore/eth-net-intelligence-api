	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		async.parallel({
			all: function (callback)
			{
				...
				batch.add(web3.net.getPeerCount.request());
				batch.add(web3.eth.getMining.request());
				batch.add(web3.eth.getHashrate.request());
				batch.add(web3.eth.getGasPrice.request());
				batch.add(web3.eth.getSyncing.request());
				var results = {};
				batch.requestManager.sendBatch(batch.requests, function(err, res) {
					if (err) {
						callback(err);
						return;
					}
					res = res || [];
					batch.requests.map(function (request, idx) {
						return res[idx] || {};
					}).forEach(function (result, i) {
						results[methods[i]] = batch.requests[i].format ? batch.requests[i].format(result.result) : result.result;
					});
		...
		function (err, results)
		{
			...
			if(results.all.peers !== null)
			{
				self.stats.active = true;
				self.stats.peers = results.all.peers;
				self.stats.mining = results.all.mining;
				self.stats.hashrate = results.all.hashrate;
				self.stats.gasPrice = results.all.gasPrice.toString(10);
